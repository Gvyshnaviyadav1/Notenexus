...
def generate_entry_filename(username):
    existing = [f for f in os.listdir(ENTRY_DIR) if f.startswith(f"{username}_entry")]
    entry_num = len(existing) + 1
    return os.path.join(ENTRY_DIR, f"{username}_entry{entry_num}.txt")
...
def handle_client(conn, addr):
    ...
    with lock:
        while username not in users or users[username]["password"] != password:
        ...
        with lock:
            if choice == "1":
                ...
                while True:
                    ...
                    lines.append(line)
                entry_text = "\n".join(lines)
                filename = generate_entry_filename(username)
                with open(filename, "w") as f:
                    f.write(entry_text)
                users[username]["diary_entries"].append(filename)
                save_data()
                conn.send(f"Diary saved as {os.path.basename(filename)}\n\n".encode())

            elif choice == "7":  # Send diary
                ...
                for i, f in enumerate(entries, 1):
                    conn.send(f"{i}. {os.path.basename(f)}\n".encode())
            ...
            # Keep other options (1–5, 8–10, 12–13) as you already have them
            # or request if you want me to include them fully again.
